...
def model_view(req):
    if req.method=='GET':
        ...
        else: return JsonResponse(s.errors)

@csrf_exempt
def update_view(req,id):
    try:
        demo=Project_use.objects.get(pk=id)
    except:
...
class fun8(APIView):
    def get(self,req,d):
        try:
            demo=Project_use.objects.get(pk=d)
            s = model_serializer(demo)
            return Response(s.data)
        except Project_use. DoesNotExist:
...
class genericapiview(generics.GenericAPIView, mixins.ListModelMixin, mixins.CreateModelMixin):
    ...
    queryset = Project_use.objects.all()
    def get(self, req):
